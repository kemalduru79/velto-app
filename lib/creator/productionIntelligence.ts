...
export type CreatorProductionTreatment = "reuse_existing" | "stock_photo" | "stock_video" | "ai_image" | "image_motion" | "ai_video";
...
export type CreatorSceneProductionDecision = {
  ...
  videoIntent: { motionImportance: number; recommendedSeconds: 5 | 7 | 10; qualityIntent: "professional" | "premium"; referenceAvailabilityCount: number; fallbackTreatment: CreatorProductionTreatment; productionPriority: number } | null;
...
const treatments: CreatorProductionTreatment[] = ["reuse_existing", "stock_photo", "stock_video", "ai_image", "image_motion", "ai_video"];
...
export function planCreatorSceneProduction(scene: CreatorProductionSceneInput, qualityTier: CreatorQualityMode): CreatorSceneProductionDecision {
  ...
  const scores: Record<CreatorProductionTreatment, number> = { reuse_existing: reuseScore, stock_photo: 0.18 + signals.stockSuitability * 0.52 + signals.authenticityValue * 0.2 - signals.motionImportance * 0.2 - signals.continuityImportance * 0.28, stock_video: 0.12 + signals.stockSuitability * 0.44 + signals.authenticityValue * 0.22 + signals.motionImportance * 0.25 - signals.continuityImportance * 0.3, ai_image: 0.28 + signals.customGenerationNeed * 0.42 + signals.continuityImportance * 0.28 - signals.motionImportance * 0.12, image_motion: 0.3 + signals.visualImportance * 0.2 + signals.continuityImportance * 0.2 + (1 - Math.abs(signals.motionImportance - 0.42)) * 0.2, ai_video: 0.08 + signals.motionImportance * 0.5 + signals.visualImportance * 0.22 + (signals.sceneRole === "climax" || signals.sceneRole === "demonstration" ? 0.12 : 0) + (signals.motionImportance > 0.82 && signals.visualImportance > 0.8 ? 0.18 : 0) - signals.stockSuitability * signals.authenticityValue * 0.18 - signals.continuityImportance * 0.08 };
  if (qualityTier === "draft") for (const treatment of treatments) scores[treatment] = treatment === "reuse_existing" && usableExisting ? 1 : treatment === "ai_image" ? 0.2 : 0;
  ...
  else selectedTreatment = treatments.reduce((best, item) => scores[item] > scores[best] ? item : best, qualityTier === "draft" ? "ai_image" : "image_motion");
  if (qualityTier === "draft" && !usableExisting) selectedTreatment = "ai_image";
  const ranked = treatments.filter((item) => item !== selectedTreatment && !(qualityTier === "standard" && item === "ai_video")).sort((a,b) => scores[b]-scores[a]);
  ...
  return { sceneId: scene.id, creatorSceneId: scene.creatorSceneId || null, qualityTier, selectedTreatment, fallbackTreatments: ranked.slice(0, 2), signals, scores: Object.fromEntries(treatments.map((key) => [key, Math.round(Math.max(0, Math.min(1, scores[key])) * 1000) / 1000])) as Record<CreatorProductionTreatment, number>, confidence: Math.round(Math.max(0.5, Math.min(0.98, 0.62 + (selectedScore-second)*0.5))*100)/100, reasonCodes, explanation,
    ...
    videoIntent: selectedTreatment === "ai_video" ? { motionImportance: signals.motionImportance, recommendedSeconds, qualityIntent: qualityTier === "cinematic" ? "premium" : "professional", referenceAvailabilityCount: Math.max(0, Number(scene.referenceAvailabilityCount || (currentImage ? 1 : 0))), fallbackTreatment: ranked.find((item) => item !== "ai_video") || "image_motion", productionPriority: Math.round((signals.visualImportance*0.45+signals.motionImportance*0.55)*100)/100 } : null,
...
export const CREATOR_PRODUCTION_TREATMENT_LABELS: Record<CreatorProductionTreatment, { en: string; tr: string }> = { reuse_existing: { en: "Existing asset", tr: "Mevcut varlık" }, stock_photo: { en: "Stock photo", tr: "Stok fotoğraf" }, stock_video: { en: "Stock footage", tr: "Stok görüntü" }, ai_image: { en: "AI visual", tr: "AI görsel" }, image_motion: { en: "Image motion", tr: "Görsel hareketi" }, ai_video: { en: "Generative motion", tr: "Üretken hareket" } };

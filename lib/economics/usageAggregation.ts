export type CreatorEconomicUsageRow = { operation_type?: unknown; provider?: unknown; model?: unknown; state?: unknown; generation_attempt?: unknown; fallback_attempt?: unknown; generated?: unknown; quantities?: unknown; actual_provider_cost_usd?: unknown; estimated_provider_cost_usd?: unknown; cost_status?: unknown };
type Bucket = { operations: number; actualCogsUsd: number; estimatedCogsUsd: number };
const record = (value: unknown): Record<string, unknown> => value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {};
const number = (value: unknown) => Number.isFinite(Number(value)) ? Math.max(0, Number(value)) : 0;
const add = (map: Record<string, Bucket>, key: string, actual: number, estimated: number) => { const item = map[key] ||= { operations: 0, actualCogsUsd: 0, estimatedCogsUsd: 0 }; item.operations += 1; item.actualCogsUsd += actual; item.estimatedCogsUsd += estimated; };
const rounded = (value: number) => Math.round(value * 1_000_000) / 1_000_000;

export function aggregateCreatorEconomicUsage(rows: CreatorEconomicUsageRow[], storage?: { activeBytes?: number; totalPhysicalBytes?: number; assetCount?: number }) {

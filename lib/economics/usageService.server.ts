...
import { aggregateCreatorEconomicUsage, collectCreatorEconomicUsagePages, deriveProjectDuration, type CreatorEconomicUsageRow } from "./usageAggregation";
...
export function getCreatorUsageWindowStart(window: CreatorUsageWindow, now = new Date()) { if (window === "project_lifetime") return null; if (window === "rolling_30_days") return new Date(now.getTime() - 30 * 86400_000).toISOString(); return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)).toISOString(); }
export async function getCreatorEconomicUsageSnapshot(input: { userId: string; projectId?: string | null; window: CreatorUsageWindow; tier?: CreatorBenchmarkTier }) {
  ...
  const start = getCreatorUsageWindowStart(input.window); const client = createServerSupabaseClient();
  const [paged, storage, account] = await Promise.all([collectCreatorEconomicUsagePages(async (from, to) => {
    let query = client.from("velto_creator_economic_operations").select("id,created_at,project_id,operation_type,provider,model,state,generation_attempt,fallback_attempt,generated,quantities,actual_provider_cost_usd,estimated_provider_cost_usd,cost_status").eq("user_id", input.userId).order("created_at", { ascending: false }).order("id", { ascending: false }).range(from, to);
    if (input.projectId) query = query.eq("project_id", input.projectId); if (start) query = query.gte("created_at", start); const { data, error } = await query; if (error) throw new Error("USAGE_AGGREGATION_FAILED"); return (data || []) as CreatorEconomicUsageRow[];
  }), getOwnerStorageQuotaStatus(input.userId), getPersistenceServices().creditRepository.getAccount(input.userId)]);
  const usage = aggregateCreatorEconomicUsage(paged.rows, storage, paged); const duration = deriveProjectDuration(project, usage); const finishedMinutes = duration.seconds === null ? null : duration.seconds / 60;

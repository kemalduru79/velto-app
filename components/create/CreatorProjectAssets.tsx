...
export default function CreatorProjectAssets({
  ...
  return (
    <>
      <details className="creatorlab-p2c-editor-disclosure" data-creator-media-storage="true">
        <summary>
          <span>{language === "en" ? "Media & Storage" : "Medya ve Depolama"}</span>
          <small>
            {storageStatus ? `${formatBytes(storageStatus.usedBytes)} · ` : ""}
            {language === "en" ? "Available" : "Kullanılabilir"} {unusedAssets.length} · {language === "en" ? "Trash" : "Çöp"} {trashedAssets.length}
          </small>
        </summary>
        <div className="creatorlab-p2c-editor-disclosure-body">
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-slate-950">
              {language === "en" ? "Media & Storage" : "Medya ve Depolama"}
            </h3>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              {language === "en"
                ? "Manage stored media, storage usage and Trash. Scene deletion is handled separately under Advanced · Scene Actions."
                : "Kayıtlı medyayı, depolama kullanımını ve Çöp Kutusunu yönetin. Sahne silme işlemi ayrı olarak Gelişmiş · Sahne İşlemleri altında yönetilir."}
            </p>
          </div>
          ...
          <section id="creator-media-management" className="scroll-mt-6" data-media-cleanup-inventory="true">
            ...
            {!cleanupLoading && !cleanupError && unusedAssets.length === 0 && (
              <p className="mt-3 rounded-xl border border-dashed border-slate-300 p-3 text-xs leading-5 text-slate-500">
                {language === "en" ? "No unused media is currently available for cleanup." : "Şu anda temizlenebilecek kullanılmayan medya bulunmuyor."}
              </p>
            )}

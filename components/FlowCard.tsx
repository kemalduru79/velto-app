// X11.9 FlowCard Stabilization Polish: safe readability and microcopy refinement.
// X11.8 FlowCard Child Readability Polish: improves old-route flow card readability and child-facing labels.
...
const statusClassMap: Record<ExperienceFlow["status"], string> = {
  ...
  pilot: "border-cyan-400/30 bg-cyan-500/10 text-cyan-800",
  coming_soon: "border-slate-400/20 bg-slate-500/10 text-slate-600",
};

const zoneClassMap: Record<FlowZone, string> = {
  AI: "border-cyan-400/30 bg-cyan-500/10 text-cyan-800",
  Maker: "border-orange-400/30 bg-orange-500/10 text-orange-800",
  VR: "border-violet-400/30 bg-violet-500/10 text-violet-800",
};
...
export default function FlowCard({ flow, language }: FlowCardProps) {
  ...
  return (
    <div className="flex h-full flex-col justify-between rounded-[28px] border border-white/70 bg-white/90 p-5 shadow-[0_12px_40px_rgba(0,0,0,0.22)] transition hover:-translate-y-1 hover:border-cyan-300 hover:bg-white">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
            ...
            <p className="mt-1 font-semibold text-slate-950">{flow.ageBand}</p>
          </div>

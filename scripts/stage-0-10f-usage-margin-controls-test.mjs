...
import { aggregateCreatorEconomicUsage, deriveProjectDuration } from "../lib/economics/usageAggregation.ts";
...
const baseAdmission = { tier: "pro", operationType: "creator_video", estimatedProviderCostUsd: .35, currentKnownCogsUsd: 10, currentVideoCogsUsd: 1, finishedMinutes: 90, costCoverageStatus: "complete", origin: "automatic" };
...
assert.equal(evaluateCreatorEconomicAdmission({ ...baseAdmission, currentKnownCogsUsd: 200, enforcementMode: "monitor" }).allowed, true);
assert.equal(evaluateCreatorEconomicAdmission({ ...baseAdmission, enforcementMode: "guard" }).allowed, true);
assert.equal(evaluateCreatorEconomicAdmission({ ...baseAdmission, currentKnownCogsUsd: 200, enforcementMode: "guard", origin: "manual" }).allowed, false);
assert.deepEqual(evaluateCreatorEconomicAdmission({ ...baseAdmission, currentKnownCogsUsd: 200, enforcementMode: "guard", qualityEquivalentFallbackAvailable: true }).reasonCodes, ["SAFE_FALLBACK_REQUIRED"]);
assert.deepEqual(evaluateCreatorEconomicAdmission({ ...baseAdmission, currentKnownCogsUsd: 200, enforcementMode: "guard", acceptableProductionFallbackAvailable: true }).reasonCodes, ["SAFE_FALLBACK_REQUIRED"]);

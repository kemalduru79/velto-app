...
const usage = [...assets.values()].filter((item) => item.owner === "a" && item.state !== "purged").reduce((sum, item) => sum + item.bytes, 0);
assert.equal(usage, 675, "usage is retained physical bytes, including Trash and independent of reference count");

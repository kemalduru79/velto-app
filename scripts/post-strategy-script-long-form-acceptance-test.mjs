...
assert.match(route, /buildCreatorEvidencePromptPack\(\{[\s\S]*context: narrationEditorialContext,[\s\S]*maxUnits: 5/);
assert.match(route, /evidenceMomentContext:[\s\S]*Control-only source-bound evidence[\s\S]*units: selectCreatorEvidencePromptPackForClaims/);
assert.match(route, /selectCreatorEvidencePromptPackForClaims\(\{[\s\S]*pack: evidenceMomentPack,[\s\S]*createSectionNativeEditorialContext/);
assert.match(route, /requestedSections\[0\]\.id === "section-3"[\s\S]*strongest_relevant_evidence_moments/);
assert.match(route, /requestedSections\[0\]\.id === "section-4"[\s\S]*relevant_counter_or_context_evidence_when_available/);
assert.match(route, /concreteEvidenceMomentGuidance: sectionNative &&[\s\S]*requestedSections\[0\]\.id === "section-3"[\s\S]*specificityClassification === "concrete_observation"/);
assert.match(route, /Realize one strongest supplied concrete evidence moment; do not substitute a generic research summary/);
assert.match(route, /supplied setup, procedure, or observed situation[\s\S]*supplied observed result[\s\S]*what that result demonstrates about this section's mechanism[\s\S]*why it matters to the episode's master question/);
assert.match(route, /research shows, studies suggest, or experiments demonstrate may introduce evidence but cannot replace the concrete evidence moment/);
assert.match(route, /Test or demonstrate the mechanism; do not restate Section 2's mechanism explanation/);
assert.match(route, /Use only details present in evidenceMomentContext\. Never invent researchers, study titles, sample sizes, dates, institutions, procedures, results, limitations, or attribution/);
assert.match(route, /concreteEvidenceMomentGuidance:[\s\S]*: null,[\s\S]*evidenceUsePriority/);
assert.match(route, /state the grounded finding and what was observed, add a boundary only when supplied/);
assert.match(route, /Never invent study details, limitations, researchers, samples, dates, numbers, procedures, or results/);
assert.match(route, /Keep evidence and source ids backstage[\s\S]*natural documentary narration rather than academic citation syntax/);

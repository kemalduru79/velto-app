...
check(changed.split("\n").filter((line) => line.includes("app/api/")).every((line) => line.endsWith("app/api/creator-video/route.ts") || line.endsWith("app/api/creator-store-video/route.ts") || line.endsWith("app/api/creator-export/route.ts")) && changed.split("\n").filter((line) => line.includes("export-service/")).every((line) => line.endsWith("export-service/src/server.js") || line.endsWith("export-service/src/creatorAudioMixPlan.js")), "only authorized video storage/export and canonical audio renderer paths changed"); // 58
...
matches(page, /creatorStageVisibility\.create_review[\s\S]*\{creatorEditorOpen && \([\s\S]*onMoveScene=\{moveSelectedCreatorEditorScene\}/, "CreatorLab Create & Review owns editor operations"); // 61

...
const liveSmoke = read("scripts/stage-0-7d-1-live-physical-purge-smoke.mjs");
...
assert.match(liveSmoke, /VELTO_LIVE_PURGE_SMOKE_CONFIRM/);
assert.match(liveSmoke, /STAGE_0_7D_1_DISPOSABLE_ONLY/);
assert.match(liveSmoke, /const generatedAssetId = randomUUID\(\)/);
assert.match(liveSmoke, /const TEST_BUCKET = "images"/);
assert.match(liveSmoke, /const TEST_PREFIX = "stage-0-7d-1-live-smoke\/"/);
assert.match(liveSmoke, /stage: TEST_STAGE, disposable: true/);
assert.match(liveSmoke, /remove\(\[generatedTestPath\]\)/);
assert.equal((liveSmoke.match(/\.remove\(/g) || []).length, 1, "smoke has one exact remove primitive");
assert.doesNotMatch(liveSmoke, /process\.argv.*(?:asset|bucket|path)|remove\(\[[^\]]*,|recursive|wildcard|emptyBucket/i);
assert.match(liveSmoke, /velto_trash_media_asset_if_unreferenced/);
assert.match(liveSmoke, /velto_begin_media_asset_purge[\s\S]*p_retention_days: 0/);
assert.match(liveSmoke, /velto_complete_media_asset_purge/);
assert.match(liveSmoke, /if \(physicalDeletionSucceeded\) \{[\s\S]*if \(!purgeCompleted\)[\s\S]*RECOVERY_REQUIRED/);
const postDeleteFailureBranch = liveSmoke.slice(liveSmoke.indexOf("if (physicalDeletionSucceeded)"), liveSmoke.indexOf("} else {", liveSmoke.indexOf("if (physicalDeletionSucceeded)")));
assert.doesNotMatch(postDeleteFailureBranch, /velto_abort_media_asset_purge|\.delete\(\)|removeExactGeneratedObject|cleanUpBeforePhysicalDeletion/);
assert.match(liveSmoke, /STAGE_0_7D_1_LIVE_PHYSICAL_PURGE_SMOKE=PASS/);
assert.doesNotMatch(liveSmoke, /VELTO_PERMANENT_MEDIA_DELETE_ENABLED|VELTO_STORAGE_QUOTA_ENFORCEMENT_ENABLED|db push|migration repair|db reset/);

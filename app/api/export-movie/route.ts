...
function resolveFfmpegBinary() {
  ...
  if (!ffmpegPath || !fs.existsSync(ffmpegPath)) {
    throw new Error(`FFmpeg binary bulunamadı: ${ffmpegPath || "empty path"}`);
  }
...
function resolveFfprobeBinary() {
  ...
  if (!ffprobePath || !fs.existsSync(ffprobePath)) {
    throw new Error(`FFprobe binary bulunamadı: ${ffprobePath || "empty path"}`);
  }
...
function getSceneTargetDuration(
  scene: ExportScene,
...
function getSceneAudioMixProfile(
  scene: ExportScene
): Required<SceneAudioMixProfile> {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const usableScenes = scenes.filter(
      (scene) =>
        scene && typeof scene.videoUrl === "string" && scene.videoUrl.trim()
    );
